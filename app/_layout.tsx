...
const navColors = {
  artistHeaderBg: '#6200ee',
  artistHeaderTint: '#fff',
  todoHeaderBg: '#fff',
  todoHeaderTint: '#212121',
  protocolHeaderBg: '#0057b7',
  protocolHeaderTint: '#ffd700',
  tabActive: '#0057b7',
  tabInactive: '#8e8e93',
};
      ...
      <Tab.Navigator
        screenOptions={({ route }) => ({
          ...
          tabBarIcon: ({ focused, color, size }) => {
            ...
            if (route.name === 'ArtistStack') {
              iconName = focused ? 'color-palette' : 'color-palette-outline';
            } else if (route.name === 'TodoStack') {
              iconName = focused ? 'list-circle' : 'list-circle-outline';
            } else if (route.name === 'ProtocolStack') {
