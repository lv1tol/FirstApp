...
import { ParticipantAFormProps } from '../../navigation/types'; // Перевірте шлях до типів
...
const ParticipantAFormScreen: React.FC<ParticipantAFormProps> = ({ navigation }) => {
  const { control, handleSubmit, formState: { errors }, setValue, watch } = useForm<FormDataA>({
    defaultValues: {
      firstName: '',
      lastName: '',
  ...
  const onSubmit = (data: FormDataA) => {
    console.log('Participant A Data:', data);
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      ...
      <Controller
        ...
        rules={{
          ...
          pattern: { value: /^\+?3?8?(0\d{9})$/, message: 'Невірний формат (+380XXXXXXXXX)' }
        }}
        render={({ field: { onChange, onBlur, value } }) => (
          <TextInput
      ...
      <Controller
        control={control}
        rules={{ required: 'Модель авто є обов\'язковою' }}
        render={({ field: { onChange, onBlur, value } }) => (
          <TextInput
      ...
      <Controller
        control={control}
        rules={{
            required: 'Номер авто є обов\'язковим',
            pattern: { value: /^[А-ЯІЇЄ]{2}\d{4}[А-ЯІЇЄ]{2}$/i, message: 'Невірний формат (АА1234АА)' }
        }}
        render={({ field: { onChange, onBlur, value } }) => (
          <TextInput
      ...
      <TouchableOpacity style={styles.submitButton} onPress={handleSubmit(onSubmit)}>
          <Text style={styles.submitButtonText}>Далі (Учасник Б)</Text>
      </TouchableOpacity>
    </ScrollView>
  );
};
...
export default ParticipantAFormScreen;